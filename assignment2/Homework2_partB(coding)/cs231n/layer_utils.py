...
def affine_batch_relu_backward(dout, cache):
    ...
    da, dgamma, dbeta = batchnorm_backward_alt(dbatch_out, batch_cache)
    dx, dw, db = affine_backward(da, fc_cache)
...
def affine_layer_relu_forward(x, w, b, gamma, beta, bn_param):
    """
    Affine - Layer Normalization - ReLU

    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    - gamma, beta: Parameters for layer normalization
    ...
    """
    a, fc_cache = affine_forward(x, w, b)
    layer_out, layer_cache = layernorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(layer_out)
...
def affine_layer_relu_backward(dout, cache):
    ...
    dlayer_out = relu_backward(dout, relu_cache)
    da, dgamma, dbeta = layernorm_backward(dlayer_out, layer_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db, dgamma, dbeta
