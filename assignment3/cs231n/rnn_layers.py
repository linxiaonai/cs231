...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        # x_t, prev_h_t, Wx_t, Wh_t, b_t, next_h_t = cache[t]
        dprev_h_t += dh[:,t,:]
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    - dprev_h: Gradient of previous hidden state, oiiiff shape (N, H)
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    N, T, H = dh.shape
    x0, _, _, _, _, _, _, _, _, _, _, _, _ = cache[0]
    _, D = x0.shape
    dx = np.zeros([N, T, D])
    dprev_h_t = np.zeros([N, H])
